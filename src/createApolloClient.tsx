...
export default function createApolloClient() {
  const httpLink = new HttpLink({
    uri: isLocalDev ? "http://localhost:4000/" : "/graphql",
    // credentials: "include",
  });

  const wsLink = new GraphQLWsWebSocketLink({
    url: isLocalDev ? "ws://localhost:4000/subscriptions" : `ws://${window.location.host}/subscriptions`,
    connectionParams: () => {
      return {};
    },
  });
  ...
  const client = new ApolloClient({
    link: ApolloLink.from([remoteLink]),
